...
def simulation_main_menu():
    ...
    while validate_user_selection[0] is False:
        ...
        user_input = __try_parse_int(input("Selection: "))
        validate_user_selection = __validate_main_menu(user_input)
    return validate_user_selection[1]


def __validate_main_menu(user_input):
...
def display_welcome():
    ...
    user_response = __continue_prompt(
...
def soda_selection(inventory):
    ...
    soda_options = __get_unique_can_names(inventory)
    while validated_user_selection[0] is False:
        ...
        for can in soda_options:
            ...
            i += 1
        user_selection = __try_parse_int(input("Selection: "))
        validated_user_selection = __validate_can_choice(
            user_selection, soda_options)
    return validated_user_selection[1]


def __validate_can_choice(selection, unique_cans):
...
def __get_unique_can_names(inventory):
...
def coin_selection():
    ...
    while validated_user_selection[0] is False:
        ...
        user_input = __try_parse_int(input("Selection: "))
        validated_user_selection = __validate_coin_selection(user_input)
        if validated_user_selection[0] is False:
            print("Not a valid selection try again")
    return validated_user_selection[1]


def __validate_coin_selection(selection):
